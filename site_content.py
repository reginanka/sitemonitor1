...
def get_schedule_content() -> Tuple[Optional[str], Optional[str]]:
    """
    Повертає (important_message, update_date) або (None, None),
    як у твоєму старому скрипті.
    """
    try:
        ...
        for elem in soup.find_all(["div", "span", "p", "h2", "h3", "h4", "h5"]):
            ...
            if (
                "УВАГА" in text
                and "ІНФОРМАЦІЯ" in text
                and important_message is None
            ):
                lines = [line.strip() for line in text.split("\n") if line.strip()]
                important_message = "\n".join(lines)
                log_to_buffer(
                    f"✅ Знайдено повідомлення УВАГА: {important_message[:100]}..."
                )
        ...
        if not important_message:
            log_to_buffer("⚠️ Повідомлення УВАГА не знайдено")
        if not update_date:
...
def take_screenshot_between_elements() -> Tuple[Optional[str], Optional[str]]:
    """
    Робить скріншот як у старому скрипті: між 'Дата оновлення інформації' та 'робіт'.
    Повертає (шлях_до_png, screenshot_hash) або (None, None).
    """
    try:
        ...
        with sync_playwright() as p:
            ...
            if not date_box:
                log_to_buffer(
                    "❌ Не вдалося отримати координати 'Дата оновлення інформації'"
                )
                browser.close()
                return None, None
