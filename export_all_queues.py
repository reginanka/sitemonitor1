...
class QueueExporter:
    ...
    def get_rems(self):
        try:
            ...
            rems = response.json()
            print(
                f"✅ Завантажено {len(rems) if isinstance(rems, list) else 1} РЕМів"
            )
    ...
    def get_cities(self, rem_id):
        try:
            response = requests.get(
                f"{API_BASE_URL}api-city.php?rem_id={rem_id}", timeout=10
            )
    ...
    def get_streets(self, city_id):
        try:
            response = requests.get(
                f"{API_BASE_URL}api-street.php?city_id={city_id}", timeout=10
            )
    ...
    def get_addresses(self, street_id):
        try:
            response = requests.get(
                f"{API_BASE_URL}api-address.php?street_id={street_id}", timeout=10
            )
        ...
        for rem_idx, rem in enumerate(rems, 1):
            ...
            for city_idx, city in enumerate(cities, 1):
                ...
                for street_idx, street in enumerate(streets, 1):
                    ...
                    street_name = street.get(
                        'name', street.get('title', f'Вулиця {street_id}')
                    )
                    ...
                    for addr_idx, addr in enumerate(addresses, 1):
                        ...
                        subqueue_id = addr.get(
                            'pidcherga_id', addr.get('subqueue_id')
                        )
                        addr_name = addr.get(
                            'name', addr.get('title', f'Адреса {addr_idx}')
                        )
                        ...
                        queue_data = {
                            ...
                            'full_address': (
                                f"{rem_name}, {city_name}, вул. "
                                f"{street_name}, {addr_name}"
                            ),
                        }
